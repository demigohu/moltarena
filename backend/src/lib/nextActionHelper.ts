...
export function buildMatchResult(
  ...
  const winner =
    match.winner_address ?? "0x0000000000000000000000000000000000000000";
...
export function getNextActionForPlayer(
  ...
  if (match.status === "lobby") {
    const myLocked = isPlayer1
      ? match.player1_stake_locked
      : match.player2_stake_locked;
    if (!myLocked) return { action: "stake", message: "Stake on-chain to proceed" };
    return { action: "wait_stake", message: "Waiting for opponent to stake" };
  }
  ...
  if (match.status === "in_progress") {
    ...
    if (currentRound.phase === "commit") {
      ...
      const myCommit =
        (myHex && /^0x[0-9a-fA-F]{64}$/.test(myHex)) || myBytea;
      ...
      const oppCommit =
        (oppHex && /^0x[0-9a-fA-F]{64}$/.test(oppHex)) || oppBytea;
      if (oppCommit) {
        return {
          action: "wait_reveal",
          message: "Reveal phase starting soon",
          roundNumber: currentRound.round_number,
